...
class StereoTrAerial4SatDatasetDenseN(Dataset):
    def __init__(self,images,mean,std,nameDataset):
        ...
        self.RightTilN=read_list_from_file(self.images+"/l_1_right_train.txt")
        self.DispTilN=read_list_from_file(self.images+"/l_1_disp_train.txt")
        self.MasqTilN=read_list_from_file(self.images+"/l_1_masq_train_fin.txt")
        
        self.LeftTilN+=read_list_from_file(self.images+"/l_2_left_train.txt")
        self.RightTilN+=read_list_from_file(self.images+"/l_2_right_train.txt")
        self.DispTilN+=read_list_from_file(self.images+"/l_2_disp_train.txt")
        self.MasqTilN+=read_list_from_file(self.images+"/l_2_masq_train_fin.txt")
        
        self.LeftTilN+=read_list_from_file(self.images+"/l_31_left_train.txt")
        self.RightTilN+=read_list_from_file(self.images+"/l_31_right_train.txt")
        self.DispTilN+=read_list_from_file(self.images+"/l_31_disp_train.txt")
        self.MasqTilN+=read_list_from_file(self.images+"/l_31_masq_train_fin.txt")
        
        self.LeftTilN+=read_list_from_file(self.images+"/l_32_left_train.txt")
        self.RightTilN+=read_list_from_file(self.images+"/l_32_right_train.txt")
        self.DispTilN+=read_list_from_file(self.images+"/l_32_disp_train.txt")
        self.MasqTilN+=read_list_from_file(self.images+"/l_32_masq_train_fin.txt")
        
        #ADDED ENSHEDE DATASET 
        self.LeftTilN+=read_list_from_file('/work/scratch/data/alichem/Data/ENSH_DENSE'+"/left_train_clean.txt")
        self.RightTilN+=read_list_from_file('/work/scratch/data/alichem/Data/ENSH_DENSE'+"/right_train_clean.txt")
        self.DispTilN+=read_list_from_file('/work/scratch/data/alichem/Data/ENSH_DENSE'+"/disp_train_clean.txt")
        self.MasqTilN+=read_list_from_file('/work/scratch/data/alichem/Data/ENSH_DENSE'+"/masq_train_clean_fin.txt")
 
        #ADDED EUROSRD DATASET DATASET 
        self.LeftTilN+=read_list_from_file('/work/scratch/data/alichem/Data/EuroSDR_vaihingen'+"/eurosdr_vahingen_left_train.txt")[:500]
        self.RightTilN+=read_list_from_file('/work/scratch/data/alichem/Data/EuroSDR_vaihingen'+"/eurosdr_vahingen_right_train.txt")[:500]
        self.DispTilN+=read_list_from_file('/work/scratch/data/alichem/Data/EuroSDR_vaihingen'+"/eurosdr_vahingen_disp_train.txt")[:500]
        self.MasqTilN+=read_list_from_file('/work/scratch/data/alichem/Data/EuroSDR_vaihingen'+"/eurosdr_vahingen_masq_train_fin.txt")[:500]
    def __len__(self):
        return len(self.LeftTilN)
    def __getitem__(self, idx):
        leftimgname=self.LeftTilN[idx]
        ...
        dispGT=self.DispTilN[idx]
        masqGT=self.MasqTilN[idx]
        imDisp=tff.imread(dispGT)*(-1.0) # mandatory no need to see it as augmentation
        imMasq=tff.imread(masqGT)/255.0
        NotOCC=imDisp*imMasq
        NotOCC=NotOCC[NotOCC!=0.0]
        D_mu=int(np.mean(NotOCC))
        ...
        if random.random()>0.5:
            ...
            # Downsampling and Upsampling 
            x00=F.interpolate(x0.unsqueeze(0).unsqueeze(0), scale_factor=SCALE_FACTOR,mode='bilinear')
            x11=F.interpolate(x1.unsqueeze(0).unsqueeze(0), scale_factor=SCALE_FACTOR,mode='bilinear')
            x0=F.interpolate(x00, scale_factor=1/SCALE_FACTOR, mode='bilinear')
            x1=F.interpolate(x11, scale_factor=1/SCALE_FACTOR, mode='bilinear')
            x0=x0.squeeze()
            x1=x1.squeeze()
        ...
        if random.random() > 0.5:
            degs,trans,scale,shears=T.RandomAffine.get_params(degrees=[0.0,0.0],
                                                              translate=None,scale_ranges=None,
                                                              shears=[-10.0,10.0],
                                                              img_size=[self.init_size,self.init_size])
            x0 = T.functional.affine(x0.unsqueeze(0),0.0,[0,0],1.0, list(shears)).squeeze()
            x1 = T.functional.affine(x1.unsqueeze(0),0.0,[0,0],1.0, list(shears)).squeeze()
            dispnoc  = T.functional.affine(dispnoc.unsqueeze(0),0.0,[0,0],1.0, list(shears)).squeeze()
            masqnocc = T.functional.affine(masqnocc.unsqueeze(0),0.0,[0,0],1.0, list(shears)).squeeze()
        # Color images to Gray 
        # Save into tensors
...
class StereoTrMontPellSatDatasetDenseN(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        dispGT=self.DispTilN[idx]
        masqGT=self.MasqTilN[idx]
        imDisp=tff.imread(dispGT)*(-1.0)*(4.0) # to correct some scaling error
        imMasq=tff.imread(masqGT)/255.0
        NotOCC=imDisp*imMasq
        NotOCC=NotOCC[NotOCC!=0.0]
        D_mu=int(np.mean(NotOCC))
...
class StereoValMontPellSatDatasetDenseN(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        dispGT=self.DispTilN[idx]
        masqGT=self.MasqTilN[idx]
        imDisp=tff.imread(dispGT)*(-1.0)*(4.0) # to correct some scaling error 264 -> 64
        imMasq=tff.imread(masqGT)/255.0
        NotOCC=imDisp*imMasq
        NotOCC=NotOCC[NotOCC!=0.0]
        D_mu=int(np.mean(NotOCC))
...
class StereoValAerialDatasetDenseN(Dataset):
    def __init__(self,images,mean,std,nameDataset):
        ...
        self.RightTilN=read_list_from_file(self.images+"/all_right_test.txt")
        self.DispTilN=read_list_from_file(self.images+"/all_disp_test.txt")
        self.MasqTilN=read_list_from_file(self.images+"/all_masq_test_fin.txt")
        #ADDED ENSHEDE DATASET 
        self.LeftTilN+=read_list_from_file('/work/scratch/data/alichem/Data/ENSH_DENSE'+"/left_test_clean.txt")
        self.RightTilN+=read_list_from_file('/work/scratch/data/alichem/Data/ENSH_DENSE'+"/right_test_clean.txt")
        self.DispTilN+=read_list_from_file('/work/scratch/data/alichem/Data/ENSH_DENSE'+"/disp_test_clean.txt")
        self.MasqTilN+=read_list_from_file('/work/scratch/data/alichem/Data/ENSH_DENSE'+"/masq_test_clean_fin.txt")
    ...
    def __getitem__(self, idx):
        ...
        # Randomly pick a region from the left image and compute the corresponding offset in the right image 
        x_upl=D_mu#+secrets.randbelow(self.init_size-self.patch_size-D_mu)
        y_upl=40#secrets.randbelow(self.init_size-self.patch_size)
        imMasq=imMasq[y_upl:y_upl+self.patch_size,x_upl:x_upl+self.patch_size]
        imDisp=imDisp[y_upl:y_upl+self.patch_size,x_upl:x_upl+self.patch_size]
        # Compute Mean of Disparity
        # Read images
...
class DDeformDataset(Dataset):
    def __init__(self,images,mean,std,nameDataset):
        ...
        self.patch_size=768
        self.ND=nameDataset
        self.LeftTilN=read_list_from_file(self.images+"/left_train.txt")
        self.RightTilN=read_list_from_file(self.images+"/right_train.txt")
        self.DispTilNX=read_list_from_file(self.images+"/disp_x_train.txt")
        self.DispTilNY=read_list_from_file(self.images+"/disp_y_train.txt")
    ...
    def __getitem__(self, idx):
        ...
        dispGTX=self.DispTilNX[idx]
        dispGTY=self.DispTilNY[idx]
        
        
        imDispX=tff.imread(dispGTX)
        imDispY=tff.imread(dispGTY)
        ...
        imDispX=imDispX[y_upl:y_upl+self.patch_size,x_upl:x_upl+self.patch_size]
        imDispY=imDispY[y_upl:y_upl+self.patch_size,x_upl:x_upl+self.patch_size]
        ...
        # Save into tensors
        x0=torch.from_numpy(grayimL[y_upl:y_upl+self.patch_size,x_upl:x_upl+self.patch_size]).unsqueeze(0).float()
        x1=torch.from_numpy(grayimR[y_upl:y_upl+self.patch_size,x_upl:x_upl+self.patch_size]).unsqueeze(0).float()
        # NORMALIZE TILES LOCALLY 
        # NORMALIZE GLOBALLY 
        x0= x0 - 0.5
        x1= x1 - 0.5
        ...
        dispnocX=torch.from_numpy(imDispX)
        dispnocY=torch.from_numpy(imDispY)
        
        return x0,x1,dispnocX,dispnocY
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++#



############################################################################################
class DDeformDatasetVal(Dataset):
    def __init__(self,images,mean,std,nameDataset):
        self.images=images
        self.meanG=mean
        self.stdG=std
        ...
        #self.patch_size=768
        self.ND=nameDataset
        self.LeftTilN=read_list_from_file(self.images+"/left_test.txt")
        self.RightTilN=read_list_from_file(self.images+"/right_test.txt")
        self.DispTilNX=read_list_from_file(self.images+"/disp_x_test.txt")
        self.DispTilNY=read_list_from_file(self.images+"/disp_y_test.txt")
    ...
    def __getitem__(self, idx):
        ...
        # Compute Mean of Disparity
        # Read images
        grayimL=tff.imread(leftimgname)/255.0  - 0.5 
        grayimR=tff.imread(rightimgname)/255.0 - 0.5 
...
def CheckWarping(batch):
    false1=0.5
    false2=1.0
    x0,x1,dispnocX,dispnocY,x_upl,y_upl=batch
    
    # do what daloader does 
    x0=x0.unsqueeze(0)
    x1=x1.unsqueeze(0)
    
    dispnocX=dispnocX.unsqueeze(0)
    dispnocY=dispnocY.unsqueeze(0)
    ...
    Offset_negX=((false1 - false2) * torch.rand(dispnocX.size(),device='cpu') + false2)
    RandSens=torch.rand(dispnocX.size(),device='cpu')
    RandSens=(RandSens < 0.5).float()+(RandSens >= 0.5).float()*(-1.0)
    Offset_negX=Offset_negX*RandSens
    
    # Random shift in y direction  
    Offset_negY=((false1 - false2) * torch.rand(dispnocY.size(),device='cpu') + false2)
    RandSens=torch.rand(dispnocY.size(),device='cpu')
    RandSens=(RandSens < 0.5).float()+(RandSens >= 0.5).float()*(-1.0)
    Offset_negY=Offset_negY*RandSens
    
    #dispnoc0=torch.nan_to_num(dispnoc0, nan=0.0)
    Index_X=torch.arange(0,dispnocX.size()[-1],device='cpu')
    Index_X=Index_X.expand(dispnocX.size()[-2],dispnocX.size()[-1]).unsqueeze(0).unsqueeze(0).repeat_interleave(x0.size()[0],0)
    
    Index_Y=torch.arange(0,dispnocY.size()[-2],device='cpu').resize(dispnocY.size()[-2],1)
    ...
    OffnX=Index_X+(dispnocX+Offset_negX)
    OffnY=Index_Y+(dispnocY+Offset_negY)
    
    
    print(OffpX.shape, OffpY.shape, OffnX.shape, OffnY.shape)
    # Clean Indexes so there is no overhead
        
    # Masking out of border regions 
    ...
    """
    AllGeoP=torch.stack((2*OffpX.squeeze(1).div(FeatsR.size()[-1]-1.0),
                         2*OffpY.squeeze(1).div(FeatsR.size()[-2]-1.0)),-1).sub(1.0)
    
    AllGeoN=torch.stack((2*OffnX.squeeze(1).div(FeatsR.size()[-1]-1.0),
                         2*OffnY.squeeze(1).div(FeatsR.size()[-2]-1.0)),-1).sub(1.0)
    ...
    tff.imwrite("Matching.tif",FeatsR_plus.detach().squeeze().numpy())
    tff.imwrite("NonMatching.tif",FeatsR_minus.detach().squeeze().numpy())
    
    
    
def CheckWarpingCircular(batch):
    false1=0.5
    false2=2.0
    rot1=0
    rot2=2*np.pi
    ...
    # Random shift in x direction 
    Offset_neg_radius=((false1 - false2) * torch.rand(dispnocX.size(),device='cpu') + false2)

    # Random shift in y direction  
    Offset_neg_angle=((rot1 - rot2) * torch.rand(dispnocY.size(),device='cpu') + rot2)
    ...
    """Train_dataset=StereoValPatchAerialDatasetDenseN('/home/ad/alichem/scratch/stereo_test/Data/DUBLIN_DENSE',0.434583236,0.1948717255,'dublin')
    lefts=[]
    rights=[]
    disps=[]
    Masqs=[]
    for id in range(Train_dataset.__len__()):
        Train_dataset.__getitem__(id,lefts,rights,disps,Masqs)
    # store positions
...
if __name__=="__main__":
    """
    DeformDataset=DDeformDataset("../SampleDatasetDeform", 0.0, 1.0, "postsism")
    # Tests 
    x0,x1,dispnocX,dispnocY,x_upl,y_upl=DeformDataset.__getitem__(0)
    
    # save image and check     
    tff.imwrite("LeftTile.tif",x0.detach().squeeze().numpy())
    tff.imwrite("RightTile.tif",x1.detach().squeeze().numpy())
    tff.imwrite("dispX.tif",dispnocX.detach().squeeze().numpy())
    tff.imwrite("dispY.tif",dispnocY.detach().squeeze().numpy())
    
    batch=x0,x1,dispnocX,dispnocY,x_upl,y_upl
    CheckWarpingCircular(batch)
    """
    _deformDataset=DDeformDataset('/home/mohamedali/Documents/1.Travaux/61.GeneralizedLearning/train',
                                  0.493,
                                  0.2,
                                  'landslide'
                                  )

    for id in range(_deformDataset.__len__()):
        if not os.path.exists(_deformDataset.LeftTilN[id]):
            print(_deformDataset.LeftTilN[id])
        if not os.path.exists(_deformDataset.RightTilN[id]):
            print(_deformDataset.RightTilN[id])
        if not os.path.exists(_deformDataset.DispTilNX[id]):
            print(_deformDataset.DispTilNX[id])
        if not os.path.exists(_deformDataset.DispTilNY[id]):
            print(_deformDataset.DispTilNY[id])
            
            
            
            
    _deformDatasetT=DDeformDatasetVal('/home/mohamedali/Documents/1.Travaux/61.GeneralizedLearning/test',
                                  0.493,
                                  0.2,
                                  'landslide'
                                  )

    for id in range(_deformDatasetT.__len__()):
        if not os.path.exists(_deformDatasetT.LeftTilN[id]):
            print(_deformDatasetT.LeftTilN[id])
        if not os.path.exists(_deformDatasetT.RightTilN[id]):
            print(_deformDatasetT.RightTilN[id])
        if not os.path.exists(_deformDatasetT.DispTilNX[id]):
            print(_deformDatasetT.DispTilNX[id])
        if not os.path.exists(_deformDatasetT.DispTilNY[id]):
            print(_deformDatasetT.DispTilNY[id])